...
export const DashboardPage: React.FC = () => {
  ...
  const [activeWidgets, setActiveWidgets] = useState<string[]>(() =>
    offlineSync.get<string[]>(LAYOUT_KEY) ?? ['kpi-issues', 'gauge-resolution']
  );
  ...
  const saveLayout = () => {
    offlineSync.set(LAYOUT_KEY, activeWidgets, 7 * 24 * 60 * 60 * 1000);
    setSaved(true);
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Welcome back, {firstName}!</Typography>
          <Typography variant="body2" color="text.secondary">
            Here's what's happening with your infrastructure today.
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button size="small" variant="outlined" startIcon={<Save />} onClick={saveLayout}>Save Layout</Button>
        ...
        {STATS.map(s => (
          <Grid item xs={6} sm={3} key={s.title}>
            <Paper sx={{ p: 2, borderTop: `4px solid ${s.color}`, borderRadius: 2 }}>
              ...
              <Typography variant="caption" sx={{ color: s.change.startsWith('+') ? 'success.main' : 'error.main' }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={8}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
              <Typography variant="h6" fontWeight={600} gutterBottom>Issue Trends</Typography>
              <Box sx={{ height: 280 }}>
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 1 }}>
                <GaugeChart value={84} label="Resolution Rate" size={120} />
                <GaugeChart value={72} label="Response Time"   size={120} />
              </Box>
            </Paper>
          </Grid>
        </Grid>
      ...
      <TabPanel value={tab} index={1}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={8}>
            <StackedBarChart labels={MONTHLY_LABELS} datasets={STACKED_DATASETS} title="Issues by Category (Monthly)" height={280} />
          </Grid>
          <Grid item xs={12} md={4}>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
              <Typography variant="h6" fontWeight={600} gutterBottom>All Performance Gauges</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 2 }}>
                ...
                ].map(g => <GaugeChart key={g.label} value={g.value} label={g.label} size={140} />)}
              </Box>
            </Paper>
          </Grid>
        </Grid>
      ...
      <TabPanel value={tab} index={2}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={7}>
            <Suspense fallback={<Loader />}><RiskAssessment /></Suspense>
          </Grid>
          <Grid item xs={12} md={5}>
            <Suspense fallback={<Loader />}><MaintenancePredictor /></Suspense>
          </Grid>
        </Grid>
      </TabPanel>

      {/* Tab 3 — Custom Widgets */}
              ...
              {activeWidgets.length === 0 && (
                <Grid item xs={12}>
                  <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 2, border: '2px dashed #e0e0e0' }}>
                    <Typography color="text.secondary">No widgets added. Use the library →</Typography>
                  </Paper>
                </Grid>
              )}
            </Grid>
      ...
      <TabPanel value={tab} index={4}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={7}><ReportBuilder /></Grid>
          <Grid item xs={12} md={5}>
            <Paper sx={{ p: 3, borderRadius: 2 }}>
